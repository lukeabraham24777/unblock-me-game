...
\IfFileExists{generated/macros_variants.tex}{\input{generated/macros_variants.tex}}{}
\IfFileExists{generated/macros_sawtooth.tex}{\input{generated/macros_sawtooth.tex}}{}
\IfFileExists{generated/macros_stepslope.tex}{\input{generated/macros_stepslope.tex}}{}
\IfFileExists{generated/macros_stepslope_tune.tex}{\input{generated/macros_stepslope_tune.tex}}{}
\IfFileExists{generated/macros_pwl.tex}{\input{generated/macros_pwl.tex}}{}
\IfFileExists{generated/macros_pilot.tex}{\input{generated/macros_pilot.tex}}{}
...
unbounded slope makes it collapse to chance at eight hidden layers. Bounding the slope and giving the
function GELU's shape with three linear pieces yields a candidate that ties GELU on every benchmark, is
depth-stable, and costs about half as much per element when compiled, but it is statistically
indistinguishable from the existing Hardswish and slower than GELU in eager execution. All code, raw
...
\begin{description}[nosep,leftmargin=1.5em]
  ...
  \item[E13: Bounded-slope candidates.] \sloppy Five piecewise-linear functions of the form
        $f(x)=\sum_i c_i\,\mathrm{ReLU}(x-t_i)$ with two to four terms, each with slope bounded by
        $\sum_i c_i$: the capped slope-stepper ($W=2$, $\delta=1$, $k$ clipped to $[-1,1]$), a soft
        knee with slopes $0/0.5/1$ over $[-1,0]$, a four-step ramp from slope $0$ to $1$ over $[-4,0]$,
        and three- and four-piece linear approximations of GELU. All are screened under the E12
        validation protocol together with GELU, ReLU, and Hardswish, the best is run head-to-head under
        the E4--E6 protocols (ReLU and GELU rows reused from E12, which used identical seeds), and the
        cost of every function is measured three ways: forward-plus-backward on a $4\times10^6$-element
        tensor in eager PyTorch, the same under \texttt{torch.compile}, and a compiled
        linear--activation--linear block with 1024 units in, hidden, and out at batch 4096. Medians of 20
...
\begin{table}[t]
  ...
  \label{tab:variants}
  \resizebox{\linewidth}{!}{\input{generated/tab_variants.tex}}
\end{table}
...
\begin{table}[t]
  ...
  \input{generated/tab_variant_occupancy.tex}
\end{table}
...
\begin{table}[t]
  ...
  network.}
  \label{tab:sawtooth}
  \resizebox{\linewidth}{!}{\input{generated/tab_sawtooth.tex}}
\end{table}
...
\begin{table}[t]
  ...
  \begin{minipage}[t]{0.62\linewidth}\centering
    \resizebox{\linewidth}{!}{\input{generated/tab_sawtooth_lr.tex}}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.34\linewidth}\centering
    \resizebox{\linewidth}{!}{\input{generated/tab_sawtooth_descent.tex}}
  \end{minipage}
\end{table}
...
\begin{table}[t]
  ...
  network.}
  \label{tab:stepslope}
  \resizebox{\linewidth}{!}{\input{generated/tab_stepslope.tex}}
\end{table}
...
\begin{table}[t]
  ...
  \resizebox{\linewidth}{!}{\input{generated/tab_stepslope_tune.tex}}
\end{table}
...
\begin{table}[t]
  ...
  \resizebox{\linewidth}{!}{\input{generated/tab_stepslope_final.tex}}
\end{table}
...
\IfFileExists{generated/tab_pwl_final.tex}{%
\subsection{Bounded-slope candidates: cheaper than GELU, and how close?}
\label{sec:pwl}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{fig_pwl.pdf}
  \caption{E13. Left: the five piecewise-linear candidates against GELU (the selected one is drawn
  solid). Middle: screening validation accuracy (large marker is the mean, small dots are seeds,
  dotted line is GELU). Right: compiled linear--activation--linear block time against final
  Fashion-MNIST MLP test accuracy for the selected candidate and the three references; whiskers are
  $\pm1$ sd over 5 seeds.}
  \label{fig:pwl}
\end{figure}
...
\begin{table}[t]
  \centering\small
  \caption{E13 screen (validation split of Fashion-MNIST, 3 epochs, 3 seeds; toy tasks as in E9).
  ``Pieces'' is the number of ReLU terms. The selected candidate is starred. ``Epoch'' is eager
  wall-clock seconds per epoch of the Fashion-MNIST MLP.}
  \label{tab:pwl-screen}
  \resizebox{\linewidth}{!}{\input{generated/tab_pwl_screen.tex}}
\end{table}

\begin{table}[t]
  \centering\small
  \caption{E13 head-to-head under the main protocols (5 seeds for the MLPs, 3 for the CNN and depth
  rows). ReLU and GELU rows are the E12 runs, which used the same seeds. Welch $p$-values compare the
  selected candidate with GELU and with Hardswish.}
  \label{tab:pwl-final}
  \resizebox{\linewidth}{!}{\input{generated/tab_pwl_final.tex}}
\end{table}

\begin{table}[t]
  \centering\small
  \caption{Kernel cost, medians of 20 repetitions: forward-plus-backward of the activation alone on
  $4\times10^6$ elements (eager and compiled), and a compiled $1024\to1024\to1024$
  linear--activation--linear block at batch 4096. The last column is the block time relative to GELU.}
  \label{tab:kernel-cost}
  \input{generated/tab_kernel_cost.tex}
\end{table}

\begin{table}[t]
  \centering\small
  \caption{End-to-end training cost: seconds per epoch of the Fashion-MNIST MLP and CNN, eager and
  with the model under \texttt{torch.compile} (timed epoch excludes 30 warm-up steps that absorb
  compilation; single run, seed 0, 4 CPU threads). The compiled ReLU CNN time is slower than its
  eager time, which is consistent with the unstable compiled ReLU backward noted in
  Table~\ref{tab:kernel-cost}; we treat it as an artefact of this PyTorch CPU build.}
  \label{tab:epoch-time}
  \input{generated/tab_epoch_time.tex}
\end{table}

The stated target for this final experiment was concrete: beat GELU on cost and at least nearly tie it
on accuracy. Section~\ref{sec:stepslope-tune} showed that the unbounded slope of the tuned
slope-stepper is what breaks it at depth and that its kernel is slower than GELU's. E13 therefore
restricts the design to functions with bounded slope that are also cheap to evaluate: sums of two to
four ReLU terms (Figure~\ref{fig:pwl}, left). Two of them are the slope-stepper with its slope capped;
the others place the kinks by hand, including two that trace the shape of GELU with straight
segments. Hardswish, the standard cheap approximation to GELU/Swish, is included as the reference that
any such design has to beat.

\paragraph{Screen.} Table~\ref{tab:pwl-screen} and Figure~\ref{fig:pwl} (middle) show that the family
is uniformly competitive: every candidate is within a point of GELU on validation accuracy, and the two
GELU-shaped ones are level with it (\pwlBestVal\% and its four-piece sibling, against \pwlGeluVal\% for
GELU and \pwlHardswishVal\% for Hardswish). The three-piece version, \pwlBest{}, is selected: it is the
cheapest of the two and the extra piece in the four-piece version buys nothing. The candidate's
defining feature is the small negative dip on $[-2.5,0]$ with a minimum of $-0.175$ at $x=-0.75$, which
GELU also has and ReLU lacks; the capped stepper and the knee, which are non-negative, are half a point
behind.

\paragraph{Accuracy.} Under the full protocol (Table~\ref{tab:pwl-final}) \pwlBest{} ties or edges
GELU on every benchmark: \pwlMnistBest\% versus \pwlMnistGelu\% on MNIST ($p=\pwlMnistPGelu$),
\pwlFashionBest\% versus \pwlFashionGelu\% on Fashion-MNIST ($p=\pwlFashionPGelu$), and
\pwlCnnBest\% versus \pwlCnnGelu\% on the CNN ($p=\pwlCnnPGelu$). None of the differences is
significant, which is the outcome ``nearly tie'' asks for. Because the slope is bounded by $1$, the
depth collapse of Section~\ref{sec:stepslope-tune} is gone: at eight hidden layers the candidate reaches
\pwlDepthBestEight\% against \pwlDepthGeluEight\% for GELU. Hardswish behaves the same way throughout,
within noise of both.

\paragraph{Cost.} Table~\ref{tab:kernel-cost} gives the kernel cost three ways, and the answer depends
on how the function is executed.
\begin{itemize}[nosep]
  \item \textbf{Eager PyTorch.} A sum of three ReLUs is three activation passes plus scaling and
        addition, each a separate kernel launch and a separate pass over memory; its forward is about
        $14\times$ slower than GELU's single fused kernel (\costBestEagerFwd{} versus
        \costGeluEagerFwd{} ms per $4\times10^6$ elements). In eager mode the candidate loses on cost
        outright, and the eager CNN epoch is $\epochBestCnnEager$ s against $\epochGeluCnnEager$ s
        (Table~\ref{tab:epoch-time}).
  \item \textbf{Compiled.} Under \texttt{torch.compile} the three terms fuse into one elementwise
        kernel, and the arithmetic then decides: a few comparisons and multiply-adds against an error
        function. The compiled forward is \costBestCompiledFwd{} ms against \costGeluCompiledFwd{} ms for
        GELU, a factor of \costBestFwdRatio, and level with ReLU (\costReluCompiledFwd{} ms) and
        Hardswish (\costHswCompiledFwd{} ms). Compiled forward-plus-backward is also below GELU's.
        (Compiled backward timings for the plain-ReLU-family kernels were unstable across repetitions
        and we do not rely on them.)
  \item \textbf{End to end.} In a compiled training loop the activation is a small share of the work,
        so the gain shrinks: the compiled CNN epoch is \epochBestCnnCompiled{} s against
        \epochGeluCnnCompiled{} s for GELU, about $\epochBestCnnRatio\%$ faster, and the MLP epochs are
        indistinguishable.
\end{itemize}

\paragraph{Verdict.} Against the stated target, \pwlBest{} succeeds conditionally. On accuracy it ties
GELU on all three benchmarks and holds up at depth. On cost it beats GELU by a factor of about
\costBestFwdRatio{} at the kernel level and by a few percent end to end, but only when the function is
compiled or otherwise fused; as a composition of PyTorch eager ops it is slower than GELU. The
qualification that matters most is that Hardswish already occupies this point in the design space: it
is at least as cheap, is a single fused kernel even in eager mode, and is statistically
indistinguishable from \pwlBest{} on every benchmark here ($p\ge0.05$ throughout
Table~\ref{tab:pwl-final}). What this experiment establishes is therefore not a new activation
function but a clean demonstration of the rule the rest of the paper converged on: a cheap
piecewise-linear function with GELU's shape, a bounded slope, and a small negative dip matches GELU's
accuracy at ReLU's cost, and the design freedom beyond that is worth very little.
}{}

\IfFileExists{generated/tab_pilot.tex}{%
\subsection{Pilot: activation $\times$ execution mode}
\label{sec:pilot}

\begin{table}[t]
  \centering\small
  \caption{$2\times2$ pilot: $\{$GELU, HardGELU-3$\}\times\{$eager, compiled$\}$ on the Fashion-MNIST
  MLP and CNN, 3 epochs, 2 seeds, batches of 128 with the last partial batch dropped. Accuracy is
  always evaluated eagerly on the shared parameters. Epoch 1 of a compiled cell includes compilation;
  ``steady epoch'' is the mean of epochs 2--3.}
  \label{tab:pilot}
  \input{generated/tab_pilot.tex}
\end{table}

The cost argument of Section~\ref{sec:pwl} rests on compilation, so a natural check is a small
factorial pilot that crosses the activation with the execution mode and confirms two things at once:
that compiling changes the speed and not the result, and that the kernel-level advantage of
\pwlBest{} survives in an actual training loop. Table~\ref{tab:pilot} reports the four cells for each
model.

\paragraph{Compilation does not change accuracy.} For GELU the compiled and eager cells agree to the
last digit on the MLP (both seeds give identical accuracies) and to $\pilotCnnGeluAccDiff$ points on
the CNN. For \pwlBest{} the differences are $\pilotMlpBestAccDiff$ points on the MLP and
$\pilotCnnBestAccDiff$ on the CNN, well inside the seed-to-seed spread. The compiled fusion of three
ReLU terms is numerically the same function.

\paragraph{The speed result depends on the model.} On the MLP the activation is a negligible share of
a step and all four cells train at $2$--$2.6$ s per epoch; eager \pwlBest{} is the slowest cell and
compiling brings it level with GELU, not ahead of it. On the CNN, where the activation runs on large
feature maps, the two execution modes reverse the ranking. Eager \pwlBest{} needs $\pilotCnnBestEagerS$ s
per epoch against $\pilotCnnGeluEagerS$ s for eager GELU, a $50\%$ penalty; compiled \pwlBest{} needs
$\pilotCnnBestCompiledS$ s against $\pilotCnnGeluCompiledS$ s for compiled GELU, $\pilotCnnCompiledRatio\%$
faster. Compilation is worth $18\%$ to GELU and $55\%$ to \pwlBest{}, because it removes the
three-pass structure that is the only reason the candidate is expensive.

\paragraph{Reading.} The pilot confirms the conditional verdict of Section~\ref{sec:pwl} with an
interaction rather than a main effect: there is no execution mode in which \pwlBest{} is worse on
accuracy, and there is exactly one setting, compiled and convolution-heavy, in which it is meaningfully
cheaper than GELU. Anyone deploying it without a compiler should expect to pay for it instead.
}{}
...
GELU is unaffected. Capping that slope and tracing GELU's shape with three linear pieces gives the one
design that meets the study's final target, tying GELU on accuracy at about half the compiled kernel
cost, though it does not improve on Hardswish, which already sits at that point. Across all of these
designs the same rule holds: an
