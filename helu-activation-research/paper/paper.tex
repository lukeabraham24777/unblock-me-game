...
and the linear control comes equally close there. An ablation over fifteen variants of the notch
(moved, deepened, or widened) shows that the deficit is a property of the notch design rather than of
its particular constants. All code, raw results, and figures are released alongside this document.
\end{abstract}
...
activation at all comes equally close there.

\section*{Reproducibility}

The code, raw JSON results for every run, figures, and this document are in the
\texttt{helu-activation-research/} directory of the repository. Running \texttt{run\_all.py}
regenerates the results (about 40 minutes on four CPU cores for the main experiments, plus about
30 minutes for the variants), \texttt{make\_figures.py} regenerates every figure and table in this
paper from them, and \texttt{make} in the \texttt{paper/} directory rebuilds the PDF.
