...
targets closely. The HeLU network produces, for every target, a straight line that is visually
indistinguishable from the linear control's fit: the least-squares line through the data. Its test MSE
is one to two orders of magnitude higher than that of ReLU or GELU. This is exactly what the null model
predicts: the network has learned an affine map, and the notch has not been recruited to bend it.
...
\accMoonsHelu\% and \accSpiralsHelu\%, matching the linear control (\accMoonsIdentity\% and
\accSpiralsIdentity\%) to within seed noise; its decision boundary is a straight line
(Table~\ref{tab:cls2d}). On spirals, where no linear classifier can do much better than chance, HeLU's
