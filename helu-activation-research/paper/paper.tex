...
\begin{description}[nosep,leftmargin=1.5em]
  ...
  \item[E9: Notch variants.] Fifteen variants of \eqref{eq:helu} obtained by moving the notch
        ($[0.5,0.6]$, $[0.1,0.2]$, $[-0.05,0.05]$), deepening it (scale $0.9$, $0.8$, $0.5$, $0.2$), or
        widening it ($[0.5,1.0]$, $[0.5,2.0]$, $[0.5,\infty)$ at scale $0.2$). Each variant is run under
...
\begin{figure}[t]
  ...
  \caption{E9: fifteen variants of the HeLU notch, written as $[\text{lo},\text{hi}]\times\text{scale}$.
  The first row is the original HeLU. Bars are means over 3 seeds, dots are seeds, and the vertical
  lines are the ReLU, GELU, and linear-control means from the main experiments. Colour groups variants
  by notch position; yellow marks the widened notches.}
  \label{fig:variants}
\end{figure}
